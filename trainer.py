...
from utils.visual import tensor2img, save_image, make_grid
from utils.filesystem import makedirs, save, load
from utils.timer import TimeAverager
from utils.logger import get_logger
from utils.visual_3d import visualize_pcd, visualize_voxel
try:
    from metrics.compute_fid_pytorch import make_fid_calculator
except:
...
class Trainer:
    ...
    def __init__(self, cfg):
        ...
        self.logger.info('Done Loading dataset [{}], num batchs: {}'.format(
            cfg.dataset.train.name, self.iters_per_epoch))
    ...
    def train(self):
        ...
        try:
            self.fid_calc = make_fid_calculator(self.train_dataloader)
        except:
            self.fid_calc = None
        ...
        while self.current_iter < (self.total_iters + 1):
            ...
            if self.validate_interval > -1 and self.current_iter % self.validate_interval == 0 and self.fid_calc is not None:
                
                self.logger.info("FID score for epoch %d: %.6f" % (self.current_iter, self.fid_calc.fid(self.model.sample_fid())))
                # np.save(self.model.sample_fid())
                # self.test()
    ...
    def test(self):
        ...
        for i in range(self.max_eval_steps):
            ...
            self.visual('visual_test',
                        visual_results=current_visuals,
                        step=self.batch_id,
                        is_save_image=True)
    ...
    def visual(self,
        ...
        for label, image in visual_results.items():
            ...
            msg = 'epoch%.3d_' % self.current_epoch if self.cfg.is_train else ''
            makedirs(os.path.join(self.output_dir, results_dir))
            img_path = os.path.join(self.output_dir, results_dir, msg + '%s.png' % (label))
            if len(image.shape) == 5: 
                # video visualization 
                image = paddle.transpose(image[0], [1, 0, 2, 3])
            # If mean offset is available, add it.
            # image += getattr(self.train_dataloader.dataset.dataset, "mean_offset", 0)
            print(image.shape)
            if len(image.shape) == 4 and image.shape[1] <= 3: 
                # Image visualization
                image = make_grid(image[:100], self.cfg.log_config.get('samples_every_row', 8)).detach()
                image_numpy = tensor2img(image, min_max, image_num)
                save_image(image_numpy, img_path)
            elif len(image.shape) == 4 and image.shape[1] > 3:
                # Voxel visualization 
                visualize_voxel(image, output_filename=img_path)
            elif len(image.shape) == 3 and image.shape[1] == 3: 
                # Point cloud visualization 
                image_numpy = np.stack((visualize_pcd(image, num_rows=4, num_cols=8),)*3, axis=-1)
                save_image(image_numpy, img_path)
